function [MVAbase, bus, gen, branch, success, et] = runpf_LB110(casename, mpopt, fname, solvedcase)
...
%   modified by TQ liu, TJ£¬SCU JICHUB LAB110 2016-10-26
%   Based on algorithm in "Senior Power System Analysis" by Zhangboming, %
%   modified core functions such as fdpf, JaccobiMatrix, newtonpf, renamed
%   as fdpf_LAB110, JaccobiMartix_LAB110, newtionpf_LAB110.
%profile on;
%%-----  initialize  -----
...
if dc                               %% DC formulation
    ...
    Pbus = real(makeSbus_LAB110(baseMVA, bus, gen)) - Pbusinj - bus(:, GS) / baseMVA;%
    ...
    [Ybus,Yt,Yf] = makeYbus_LAB110(baseMVA, bus, branch); %modified version of function makeYbus in matpower 3.03b
    
    %% compute complex bus power injections (generation - load)
    Sbus = makeSbus_LAB110(baseMVA, bus, gen);  %modified version of function makeSbus in matpower 3.03b
  %  Sbus = makeSbus(baseMVA, bus, gen);
    
    %% run the power flow
    alg = mpopt(1);
    if alg == 1
        [V, success, iterations] = newtonpf_LAB110(Ybus, Sbus, V0, ref, pv, pq,nb,mpopt);%modified version of function newtionpf in matpower 3.03b
        %[V, converged, i] = newtonpf_LB110(Ybus, Sbus, V0, ref, pv, pq,G,B, nb,mpopt)
    elseif alg == 2 | alg == 3
        [Bp, Bpp] = makeB_LAB110(baseMVA, bus, branch, alg);%modified version of function makeB in matpower 3.03b
        [V, success, iterations] = fdpf_LAB110(Ybus, Sbus, V0, Bp, Bpp, ref, pv, pq, mpopt); %modified version of function fdpf in matpower 3.03b
    elseif alg == 4
...
printpf(baseMVA, bus, gen, branch, [], success, et, 1, mpopt);%ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
